...
import (
	"fmt"
	product "go-api/app/product/handler"
	ProductInterface "go-api/app/product/service"
...
func NewProductHandler(router *echo.Echo, ps ProductInterface.IProductService) {
	h := &product.ProductHandler{
		Service: ps,
	}

	product := router.Group("/product", authorizeMiddleware)
	{
		product.GET("/list", h.ListProduct)
		product.GET("/list/:id", h.ListProductByID)
		product.POST("/create", h.CreateProduct)
		product.POST("/edit", h.EditProduct)
